...
void inicializar_banco(tp_pergunta banco_perguntas[]) {
    ...
    char *ops1[] = {"&", "*", "->", "#"};
    init_pergunta(&banco_perguntas[0], "Qual operador e usado para obter o endereco de uma variavel?", 1, 'A', "Ponteiros");
    init_opcoes(&banco_perguntas[0], ops1);
    banco_perguntas[0].id = 1;

    // Pergunta 2: Filas
    char *ops2[] = {"Push", "Pop", "Enqueue", "Top"};
    init_pergunta(&banco_perguntas[1], "Qual o termo tecnico para inserir um elemento em uma Fila?", 1, 'C', "Filas");
   ...
   init_pergunta(&banco_perguntas[2], "Qual destas e uma aplicacao classica de uma Pilha?", 2, 'A', "Pilhas");
    ...
    init_pergunta(&banco_perguntas[3], "Ao somar 1 a um ponteiro (p++), quanto o endereco avanca?", 2, 'B', "Ponteiros");
    init_opcoes(&banco_perguntas[3], ops4);
    banco_perguntas[3].id = 4;

    // Pergunta 5: Recursão
    char *ops5[] = {"Heap", "Stack (Pilha)", "Data Segment", "BSS"};
    init_pergunta(&banco_perguntas[4], "Em qual regiao da memoria as chamadas recursivas sao armazenadas?", 3, 'B', "Recursao");
    ...
    init_pergunta(&banco_perguntas[5], "O que a expressao '**ponteiro' (ponteiro para ponteiro) acessa?", 3, 'C', "Ponteiros");
    ...
    init_pergunta(&banco_perguntas[6], "Os algoritmos de backtracking sao caracterizados por geralmente usarem:", 1, 'A', "Backtracking");
    ...
    init_pergunta(&banco_perguntas[7], "Uma lista simplesmente encadeada para ser devidamente inicializada deve apontar para:", 1, 'C', "Listas");
    ...
    init_pergunta(&banco_perguntas[8], "Qual das opcoes abaixo nao eh um membro de uma lista duplamente encadeada, seja ela CLASSICA ou MODERNA?", 2, 'D', "Listas");
    ...
    init_pergunta(&banco_perguntas[9], "Qual dos algoritmos abaixo segue a ideia usar árvores para realizar uma ordenação em um vetor?", 3, 'B', "Ordenação");
    ...
    init_pergunta(&banco_perguntas[10], "A busca binaria, um famoso algoritmo que divide um vetor em subvetores, costuma ter quais complexidades algoritmicas em seus pior, melhor e medio caso:", 2, 'A',"Busca");
    ...
    init_pergunta(&banco_perguntas[11], "O tipo de dado 'FILE' pertence à biblioteca:", 3, 'A',"Arquivos");
...
int realizar_pergunta(tp_item_pilha pergunta, char *resposta_marcada){
